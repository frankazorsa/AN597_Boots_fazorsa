...
Peer commentary from nicfall:
Awesome job, looks like you've got some interesting code here. I would just maybe add a few comments here & there with a # so that someone new to your code would be able to follow your answers to Chris's questions & what your code is doing a bit easier. Like I'm not sure what the goodline function parts are for? The 'results' part starting on line 90 didn't work on my computer, looks like you need () around the variables Home range & Body mass after "log". 
...
```
#Challenges
1. I am still adjusting details to my code for both questions.
2. Also, I am still working on the extra credit functions.
...
goodline <- function(x,a,b, width = 1, colour = "black", type = 1){
  ...
  y1 <- a + b*x1
  y2 <- a + b*x2
  lines(c(x1,x2),c(y1,y2), lwd = width, col = colour, lty = type)
}
```
```{r}
#abline(a = -9.44, b = 1.04, lwd = 1.5, col ="grey35")#This does not work, I am not sure why.
...
goodline(log(x = d$Body_mass_female_mean), a = -9.44, b = 1.04, colour = "grey35", width = 1.5)
```
```{r}
attributes(mod)
```
